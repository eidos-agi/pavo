...
class BatchDoctorTests(unittest.TestCase):
    def test_readme_documents_batch_handoff_operator_gate(self):
        ...
        self.assertIn("priority_queue", readme)
    ...
    def test_batch_proof_writes_doctor_verification_and_proof_reports(self):
        ...
        self.assertIn("decision_group\tdecision\tspeaker\treview_reason\tcluster_id\tquestion", proof_decision_slate)
    ...
    def test_batch_apply_decision_board_audit_cli_updates_review_slate(self):
        ...
        self.assertEqual(report["applied_row_count"], 2)
        self.assertEqual(report["approved_row_count"], 2)
        self.assertIn("D01\tapproved\tDaniel Audit\tapproved_clean_match", decision_text)
        self.assertIn("1\tS1\tapproved\tDaniel Audit", applied_text)
        self.assertIn("approved_clean_match", applied_text)

    def test_batch_apply_decision_board_audit_requires_reason_for_final_decision(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_processed_batch(root, recording_ids=["rec-a"])
            result = prove_batch(root, refresh_cluster_gate=False)
        ...
        self.assertIn("invalid or missing review_reason", stderr.getvalue())
        self.assertFalse(applied_slate.exists())
    ...
    def test_batch_decision_board_cli_writes_review_html(self):
        ...
        self.assertIn("reviewContext", html)
        self.assertIn("contextVersion", html)
        self.assertIn('"decision_risk": "high"', html)
        self.assertIn('"suggested_review_action"', html)
        self.assertIn('"evidence_hints"', html)
        ...
        self.assertIn("Review reason", html)
        self.assertIn("approved_clean_match", html)
        self.assertIn("updateReviewReason", html)
        self.assertIn("setDecisionWithReason", html)
        self.assertIn("Approve clean", html)
    ...
    def test_batch_verify_decision_board_cli_passes_for_fresh_board(self):
        ...
        self.assertIn("has_audit_review_context", {check["name"] for check in report["checks"] if check["passed"]})
    ...
    def test_batch_review_pack_cli_copies_review_artifacts_without_audio(self):
        ...
        self.assertIn("review_completion_json", artifact_names)
        self.assertIn("review_completion_markdown", artifact_names)
        self.assertIn("active_correction_status_json", artifact_names)
        self.assertIn("active_correction_status_markdown", artifact_names)
        self.assertIn("speaker_answer_sheet_markdown", artifact_names)
        self.assertIn("speaker_answer_sheet_tsv", artifact_names)
        self.assertIn("speaker_suggestions_json", artifact_names)
        self.assertIn("speaker_suggestions_markdown", artifact_names)
        self.assertIn("speaker_suggestions_tsv", artifact_names)
        ...
        self.assertIn("pavo batch finalize-board-audit", readme)
        self.assertIn("pavo batch readiness", readme)
        self.assertIn("pavo batch review-completion", readme)
        self.assertIn("pavo batch verify-review-completion", readme)
        self.assertIn("pavo batch active-correction-status", readme)
        self.assertIn("pavo batch review-cockpit", readme)
        self.assertIn("pavo batch verify-review-cockpit", readme)
        self.assertIn("pavo batch speaker-suggestions", readme)
        self.assertIn("pavo batch verify-speaker-suggestions", readme)
        self.assertIn("pavo batch speaker-calibration", readme)
        self.assertIn("pavo batch score-speaker-agreement", readme)
    ...
    def test_batch_speaker_suggestions_cli_writes_human_gated_hypotheses(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_processed_batch(root, recording_ids=["rec-a"])
            result = prove_batch(root, refresh_cluster_gate=False)
            ...
            stdout = io.StringIO()
            with redirect_stdout(stdout):
                exit_code = main(["batch", "speaker-suggestions", str(result.proof_report_path), "--json"])

            report = json.loads(stdout.getvalue())
            suggestions_json = json.loads(Path(report["json_path"]).read_text())
            markdown = Path(report["markdown_path"]).read_text()
            tsv = Path(report["tsv_path"]).read_text()
            verify_stdout = io.StringIO()
            with redirect_stdout(verify_stdout):
                verify_exit_code = main(["batch", "verify-speaker-suggestions", str(result.proof_report_path), "--json"])
        ...
        self.assertEqual(report["pending_decision_count"], 1)
        self.assertEqual(report["must_listen_count"], 1)
        self.assertEqual(suggestions_json["suggestions"][0]["decision_group"], "D01")
        self.assertEqual(suggestions_json["suggestions"][0]["machine_suggested_decision"], "pending")
        self.assertTrue(suggestions_json["suggestions"][0]["human_required"])
        self.assertEqual(suggestions_json["review_triage"]["total_decisions"], 1)
        self.assertEqual(suggestions_json["review_triage"]["risk_counts"]["high"], 1)
        self.assertEqual(suggestions_json["review_triage"]["next_actions"][0]["decision_group"], "D01")
        self.assertEqual(suggestions_json["review_triage"]["stop_rule"]["name"], "high_risk_and_identity_boundary")
        self.assertEqual(suggestions_json["review_triage"]["stop_rule"]["minimum_decision_count"], 1)
        self.assertIn("Pavo Speaker Suggestions", markdown)
        self.assertIn("Review Triage", markdown)
        self.assertIn("Stop rule", markdown)
        self.assertIn("Next best decisions", markdown)
        self.assertIn("Machine suggested decision", markdown)
    ...
    def test_batch_review_completion_cli_writes_and_verifies_gate_report(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stdout = io.StringIO()
            with redirect_stdout(stdout):
                exit_code = main(["batch", "review-completion", str(result.proof_report_path), "--json"])
            ...
            verify_stdout = io.StringIO()
            with redirect_stdout(verify_stdout):
                verify_exit_code = main(["batch", "verify-review-completion", str(result.proof_report_path), "--json"])
            verify_report = json.loads(verify_stdout.getvalue())
        ...
        self.assertEqual(report["pending_decision_count"], 1)
        self.assertEqual(payload["review_completion"]["pending_decision_count"], 1)
        self.assertIn("Pavo Review Completion", markdown)
        self.assertIn("Pending decisions: `1`", markdown)
    ...
    def test_batch_review_bundle_cli_fingerprints_launch_artifacts(self):
        ...
        self.assertIn("active_correction_status_json", payload["artifacts"])
        self.assertIn("active_correction_status_markdown", payload["artifacts"])
        self.assertIn("speaker_suggestions_tsv", payload["artifacts"])
        self.assertIn("review_pack_zip", payload["artifacts"])
        self.assertIn("active_correction_status", payload)
        self.assertFalse(payload["active_correction_status"]["stop_rule_satisfied"])
        self.assertIn("Pavo Review Bundle", markdown)
        self.assertIn("Review completion", markdown)
        self.assertIn("Active correction status", markdown)
        self.assertIn("does not approve speaker identity", markdown)
    ...
    def test_batch_review_rehearsal_cli_writes_ready_to_review_report(self):
        ...
        self.assertIn("speaker_calibration_tsv", rehearsal_json["artifacts"])
    ...
    def test_batch_review_cockpit_cli_writes_and_verifies_html(self):
        ...
        self.assertIn("Speaker suggestions", html)
        self.assertIn("Machine suggestion", html)
        self.assertIn("Human required", html)
        self.assertIn("Must-listen decisions", html)
        self.assertIn("Review triage", html)
        self.assertIn("risk_lane_blockers_then_queue_order", html)
        self.assertIn("high_risk_and_identity_boundary", html)
    ...
    def test_batch_review_sprint_cli_writes_json_and_markdown_packet(self):
        ...
        self.assertEqual(sprint_json["review_sprint"]["priority_queue"]["strategy"], "risk_then_priority_score")
        self.assertEqual(sprint_json["review_sprint"]["priority_queue"]["risk_counts"]["high"], 1)
        self.assertEqual(sprint_json["review_sprint"]["priority_queue"]["items"][0]["cluster_id"], "S1")
        self.assertIn("why_queued", sprint_json["review_sprint"]["priority_queue"]["items"][0])
        self.assertIn("Pavo Batch Review Sprint", sprint_markdown)
        self.assertIn("Priority Queue", sprint_markdown)
        self.assertIn("risk then priority score", sprint_markdown)
        self.assertIn("Risk counts: high `1`", sprint_markdown)
    ...
    def test_batch_verify_review_sprint_cli_passes_for_fresh_packet(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            _write_processed_batch(root, recording_ids=["rec-a"])
            result = prove_batch(root, refresh_cluster_gate=False)
        ...
        self.assertIn("has_priority_queue", "\n".join(check["name"] for check in report["checks"]))
    ...
    def test_batch_verify_review_sprint_cli_fails_when_markdown_loses_checklist(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            markdown_path.write_text(markdown_path.read_text().replace("Listen checklist", "Listen removed"))
    ...
    def test_batch_review_now_cli_prepares_verified_launch_packet(self):
        ...
        self.assertIn("review_triage_strategy: risk_lane_blockers_then_queue_order", text)
        self.assertIn("review_stop_rule: high_risk_and_identity_boundary first 1 decisions", text)
        self.assertIn("active_correction_status:", text)
        self.assertIn("pavo-batch-active-correction-status.md", text)
        self.assertIn("active_correction_stop_rule_satisfied: false", text)
        self.assertIn("active_correction_next_action: review stop-rule decision group D01", text)
        self.assertIn("review_first_action: D01/S1->Daniel high must_listen", text)
        self.assertIn("speaker_answer_sheet:", text)
        self.assertIn("pavo-batch-speaker-answer-sheet.md", text)
        self.assertIn("review_bundle:", text)
        self.assertIn("pavo-batch-review-bundle.md", text)
        self.assertIn("review_completion:", text)
        self.assertIn("pavo-batch-review-completion.md", text)
        self.assertIn("speaker_suggestions:", text)
        self.assertIn("pavo-batch-speaker-suggestions.md", text)
    ...
    def test_batch_review_now_cli_json_reports_verified_launch_packet(self):
        ...
        self.assertEqual(report["priority_queue_top"][0]["cluster_id"], "S1")
        self.assertEqual(report["priority_queue_top"][0]["decision_risk"], "high")
        self.assertIn("why_queued", report["priority_queue_top"][0])
        self.assertEqual(report["speaker_review_triage"]["stop_rule"]["name"], "high_risk_and_identity_boundary")
        self.assertEqual(report["speaker_review_triage"]["next_actions"][0]["decision_group"], "D01")
        self.assertFalse(report["active_correction_status"]["stop_rule_satisfied"])
        self.assertTrue(report["active_correction_status_path"].endswith("pavo-batch-active-correction-status.md"))
        ...
        self.assertTrue(report["review_bundle_written"]["passed"])
        self.assertTrue(report["review_bundle_verification"]["passed"])
        self.assertTrue(report["review_bundle_path"].endswith("pavo-batch-review-bundle.md"))
        self.assertTrue(report["review_completion_verification"]["passed"])
        self.assertTrue(report["review_completion_path"].endswith("pavo-batch-review-completion.md"))
        self.assertTrue(report["speaker_suggestions_verification"]["passed"])
        self.assertTrue(report["speaker_suggestions_path"].endswith("pavo-batch-speaker-suggestions.md"))
    ...
    def test_batch_verify_review_pack_cli_passes_for_fresh_pack(self):
        ...
        self.assertTrue(any(check["name"] == "has_review_completion_json" and check["passed"] for check in report["checks"]))
        self.assertTrue(any(check["name"] == "has_speaker_suggestions_json" and check["passed"] for check in report["checks"]))
...
def _write_processed_batch(root: Path, *, recording_ids: list[str]) -> None:
    ...
    (root / "clip.mp3").write_bytes(b"fake review clip")
