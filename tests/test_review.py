...
class ReviewTests(unittest.TestCase):
    ...
    def test_cluster_review_status_reports_pending_queue(self):
        ...
        self.assertIn("validate_slate:", markdown)
        self.assertIn("finish_from_slate:", markdown)
    ...
    def test_cluster_review_decision_brief_exports_ranked_impact_readout(self):
        ...
        self.assertIn("Validate reviewed TSV", markdown)
        self.assertIn("pavo review clusters validate-slate", markdown)
        self.assertIn("Validate reviewed TSV before mutation", html)
        self.assertIn("Safe handoff is: export TSV", html)
        self.assertIn("pavo review clusters validate-slate", html)
    ...
    def test_cluster_review_slate_import_updates_review_sheet(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cells[header.index("note")] = "sounds like Alex after listening"
            filled = slate.tsv_path.with_name("filled-slate.tsv")
            filled.write_text("\n".join([lines[0], "\t".join(cells)]) + "\n")

            imported = import_cluster_review_slate(prepared.review_sheet_path, filled)
            sheet = json.loads(prepared.review_sheet_path.read_text())
            decision = export_cluster_question_decisions(prepared.review_sheet_path)
            report = json.loads(decision.report_path.read_text())

        self.assertEqual(imported.applied_count, 1)
        self.assertEqual(imported.approved_count, 1)
        self.assertEqual(imported.pending_count, 0)
        self.assertEqual(sheet["rows"][0]["status"], "approved")
        self.assertTrue(sheet["rows"][0]["approved"])
        self.assertEqual(sheet["rows"][0]["target_speaker_label"], "Alex")
    ...
    def test_create_anchor_review_page_embeds_clip_audio_and_correction(self):
        ...
        self.assertIn('id="export-slate-tsv"', html)
        self.assertIn("buildReviewedSlateTsv", html)
        self.assertIn("exportReviewedSlateTsv", html)
        self.assertIn("pavo-cluster-review-decision-slate.tsv", html)
        self.assertIn("clusterFinishFromSlateCommand", html)
        self.assertIn("pavo review clusters finish-from-slate", html)
        self.assertIn("/api/review-sheet", html)
        self.assertIn("Record note", html)
        self.assertIn("Parse note", html)
